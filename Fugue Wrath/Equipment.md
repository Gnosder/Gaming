...
Each small or medium sized creature can carry a number of bulk equal to 5 + their strength before becoming encumbered and a maximum of 10 + their strength before being unable to move.

For each size category larger than medium the creature's bulk is multiplied by 10.
...
When a tool is used improperly, poorly over a short time (days), used without maintenance (months), used with maintenance (years) it has a chance to break. Each time an item has a chance to break roll 1d6, on a 1 the item breaks and must be replaced or repaired before it can be used again.
...
Compass: Grants +2 bonus to navigation checks and automatically determine direction of travel. Note non-magical compasses rely on there being a magnetic pole. If there is not or if something interfers with it, the compass is useless.
...
Ration: An abstracted item reprsenting appropriate food and water for a creature. Creatures consume a number of rations per day based on their size with medium creatures consuming 10. Creatures who consume fewer rations than required recieve a -2 penalty to actions until they get 3 days of normal rations.
...
Tent: Pup tents are large enough to hold only a single small or medium creature and keep them reasonably protected from the elements. Sleeping in a tent allows a creature to refresh 1 FP per night.
...
| Animal       | Price | Encumbered Capacity | Movement Speed |
| ------------ | ----- | ------------------- | -------------- |
| Donkey*      | 25sp  | 70b                 | 30ft           |
| Mule         | 50sp  | 100b                | 40ft           |
| Horse, Work  | 80sp  | 80b                 | 50ft           |
| Horse, Draft | 200sp | 120b                | 40ft           |
| Cart         | 50sp  | +50b                | 40ft           |
| Wagon        | 150sp | +50b                | 40ft           |
Vehicles like carts and wagons need to be pulled by a creature and increase its capacity by the listed amount. While only 1 creature can pull a cart, multiple can pull a wagon. Vehcicles have a max speed they can move at.
